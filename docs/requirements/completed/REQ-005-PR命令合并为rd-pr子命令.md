...
- [x] 测试点3：创建 PR 路径不读取审查 / 评论 / 合并共享文件（2026-09-15 #80 创建流程未读取 `pr-ops.md`）
- [ ] 测试点4：`review` 大 PR 仍调用原生 `/code-review`，`--level`、`--auto` 生效（待观察：#80 为小 PR，走主会话内联审查，未触发原生审查）
- [ ] 测试点5：自然语言调度器「审 PR / 合并 PR / 拉 PR 评论」映射到新子命令（待观察：调度器映射表已核对，自然语言触发未实测）
...
- [ ] 验收项2：在一个功能分支上依次执行 `/rd:pr`、`/rd:pr review`、`/rd:pr merge`，完成创建、审查、合并全流程（部分：#80 创建、status、review 通过；merge 前置检查通过，但 gh 未登录无法合并，且 merge 小节未定义 gh 不可用时的处理，改为手动 squash 合并，另开快速修复跟进）
- [x] 验收项3：`/rd:pr` 创建成功后的提示与 `/rd:fix --auto` 结束提示中，下一步均为 `/rd:pr review`（#80 实测创建提示；`fix.md` 结束提示已核对）
