...
- [ ] 测试点1：`/rd:new-quick` 确认方案后状态为「草稿」，结束提示为 `/rd:dev`，不切分支、不改代码、不 `git mv`（未实测：本轮无新建 QUICK 场景，下一个 QUICK 创建时验证）
- [x] 测试点2：`/rd:test QUICK-XXX` 读取「验证方式」逐项引导，完成后状态「测试中」，勾选写回文档（2026-09-16 QUICK-006 / QUICK-007 实测）
- [x] 测试点3：对「开发中」的 QUICK 或 REQ 执行 `/rd:done` → 均提示先 `/rd:test`；对「测试中」的 → 正常归档（正例：QUICK-006 / QUICK-007 / REQ-006 三条按「测试中」门槛归档；负例按 `done.md` 前置检查文本核对）
- [x] 测试点4：「验证方式」含裸未勾项 → 要求确认；仅含带标注未勾项 → 放行并列出（本文档归档时 4 项带标注未勾项放行并列出；裸未勾项路径按文本核对）
- [x] 测试点5：归档后索引与 `active/`、`completed/` 一致（方案 A：归档后按 `/rd:req` 规则渲染，三条落在已完成、进行中为空，守卫通过）
- [x] 测试点6：`/rd:status QUICK-XXX` 与 `/rd:req` 正确展示 QUICK（归档后 `/rd:req` 渲染含 QUICK-006 / 007；`status` 按文本核对）
- [x] 测试点7：守卫——人为把 `active/` 某文档状态改为「已完成」、或把「测试中」文档移到 `completed/`、或复制一个重复编号 → 各被报出并退出 1；恢复后通过（临时副本上实测 5 类负例：active 已完成、completed 开发中、重复编号、勾选不连续、readonly 跳过）
- [ ] 测试点8：`/rd:upgrade` 对「开发中」QUICK 生成 REQ，状态「开发中」，评审记录含「升级自 QUICK-XXX，未经评审」（待观察：本轮无升级场景）
- [x] 测试点9：`check-layout.py --check`、新守卫、diag 冒烟、CI 全部通过（#87 与 v4.1.0 发版提交的 layout-and-smoke 含新守卫步骤均 success）

### 6.2 验收标准

> 产品/业务方验收时的确认项，描述可观测的业务结果

- [ ] 验收项1：一条 QUICK 用 `new-quick → dev → test → done` 四条命令走完，全程不手工改文件（待观察：QUICK-006 / 007 创建于旧流程，仅 test → done 段在新链上实测；下一个 QUICK 全程验证）
- [x] 验收项2：归档完成后 `/rd:req` 中该需求出现在已完成分组，不再出现在进行中
- [x] 验收项3：仓库内不存在「声明自动维护但无人写入」的索引文件（`INDEX.md`、`index-template.md` 已删）
- [x] 验收项4：根 `CLAUDE.md`、README / 教程三语中 QUICK 生命周期描述为四态且与实现一致
- [ ] 验收项5：CI 在状态与目录不一致的 PR 上失败（待观察：本地负例已验证守卫退出 1，CI 层面等首个不一致 PR 出现时观察）
...
|-----|---------|---------|
| 2026-09-16 | 初始版本 | - |
| 2026-09-16 | `/rd:test`（rd 5.1.0）：回归三项通过；测试点 2 / 7 / 9、验收项 3 / 4 实测勾选，测试点 3–6 与验收项 2 待本轮 `/rd:done` 实测，其余标注待观察 | 六章 |
