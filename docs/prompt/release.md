...
- `marketplace.json` → `metadata.version`（X.Y.Z 格式，去掉 v 前缀）
...
- 运行测试：`npm test`
- 确认 `marketplace.json` 和各 `plugins/*/plugin.json` 无未提交的手动版本号修改（版本号由 `/req:release` 步骤 2.5 自动 bump，手动修改会与自动 bump 冲突）
