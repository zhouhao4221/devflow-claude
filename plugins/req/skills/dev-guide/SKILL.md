...
1. **关联后端 REQ**：九、关联信息中的关联需求 → 读取其 10.2 API 设计
...
根据实现步骤（10.4）生成 TodoWrite 任务列表，每个步骤对应一个任务。
...
| 实现步骤/文件清单有调整 | 十、实现方案 | 直接更新，无需确认 |

### 修改规则

- **一~六章**：修改前提示用户确认，确认后同时在「八、变更记录」追加一条记录（日期、变更内容、影响范围）
- **十章**：开发过程中可直接更新（步骤调整、文件增减等属于实现细节）
- **格式约束不变**：修改仍须遵循模板结构
