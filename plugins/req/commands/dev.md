...
- **10.2 API 设计**：基于第五章接口需求 + 项目代码 + CLAUDE.md API 风格，生成具体接口方案（路径、方法、请求/响应字段、错误码）
- **10.3 文件改动清单**：按 CLAUDE.md 分层架构表的顺序列出需要新增/修改的文件
- **10.4 实现步骤**：按 CLAUDE.md 分层架构的顺序拆解开发步骤
...
1. 首次进入 → 状态改为「开发中」
2. 将实现方案写回需求文档的「十、实现方案」章节（10.1 数据模型、10.2 API 设计、10.3 文件改动清单、10.4 实现步骤）
