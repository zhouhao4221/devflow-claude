...
| 分支策略 | 推荐起步分支 | 流程模式 | 原因 |
|---------|-------------|---------|------|
| `git-flow`(develop 起步) | `developBranch`(通常是 develop) | `cross-branch` | 一个 PR(develop → main)带齐 changelog + SQL + 回滚,合并后 tag |
| `git-flow`(发布分支起步) | `release/<version>` / `chore/release-<version>` | `release-branch` | 双 PR 流程(release → main 先合 + 打 tag,release → develop 回流)。适用于 develop 上已积累未准备发布的 feature,需要隔离发布内容的场景 |
| `github-flow` | `mainBranch`(通常是 main) | `direct` | main 即开发主干,direct 模式一把过 |
| `trunk-based` | `mainBranch` | `direct` | 同上 |

**何时选 cross-branch vs release-branch**:如果 develop 到 main 的 delta 就是本次要发的全部内容(没有未准备发布的 feature 堆积),cross-branch 更简单;如果 develop 上还有未就绪的 feature,必须用 release-branch 把要发的部分隔离到专用分支,否则 tag 会带上未准备发布的代码。

若在 git-flow 的主分支上运行本命令,step 1.5 的守门规则会提醒切到 develop 或 release 分支,避免 `chore(release): prepare` 提交被保护分支打回后手动绕路。
...
| 当前分支 | 流程模式 |
|---------|---------|
| `mainBranch`（如 main/master） | **direct**（当前分支即 tag 目标,无 PR） |
| `release/*` / `chore/release-*` | **release-branch**（双 PR: release→main 先合+打 tag,release→develop 回流） |
| `developBranch`（如 develop） | **cross-branch**（单 PR: develop→main 合并后在主分支打 tag） |
...
> - **release-branch 模式**:适合 develop 上已积累未准备发布的 feature 的场景,通过 release 分支隔离要发的内容
> - **cross-branch 模式**:适合 develop 到 main 的 delta 就是要发的全部内容的场景
> - 两种模式都**不在起始分支上打 tag**,tag 最终落在主分支上
...
if current_branch == main_branch:
    ...
    release_branch = current_branch   # 供 step 8.7 / 10.6 / start_branch 对照使用
...
start_branch = current_branch   # 供 step 10.5 最终切回使用
...
if (
    ...
    print()
    print(f"    推荐做法(二选一,看团队习惯):")
    print(f"      A) 切到 {develop_branch} 重新运行:走 cross-branch 流程")
    print(f"         (一个 PR: {develop_branch}→{main_branch},合并后 tag)。")
    print(f"         适合 {develop_branch} 到 {main_branch} 的 delta 就是本次要发的全部内容的场景。")
    print(f"      B) 切到/新建 release/<version> 或 chore/release-<version> 分支:走 release-branch 流程")
    print(f"         (双 PR: release→{main_branch} 先合+打 tag,release→{develop_branch} 回流)。")
    print(f"         适合 {develop_branch} 上已有未准备发布的 feature 堆积,需要隔离要发内容的场景。")
    print()
    print("请选择：")
    print(f"  [1] 切到 {develop_branch} 并中止本次运行（cross-branch,最常见;需重新执行命令）")
    print(f"  [2] 继续在 {main_branch} 直接发布（仅当主分支无保护或你确信可直推时选择）")
    print("  [3] 中止(选 release-branch 路径时选此项,手动建分支后重跑)")
    choice = input("> ").strip()
    if choice == "1":
...
**`--no-draft` 在 developBranch 或 release 分支的额外确认**:
...
if flow_mode in ("cross-branch", "release-branch") and not is_draft:
    ...
    print("   这会跳过 draft 闸门:PR 合并后立即在主分支创建 tag + 对外 Release")
...
- `flow_mode`：`direct` / `cross-branch` / `release-branch`（step 1.5 已确定）
- `is_draft`：step 1.5 已确定
- `repo_type`、`main_branch`、`develop_branch`、`release_branch`(仅 release-branch 模式)：step 1.5 已确定
...
├── ✅ 创建 PR1: <release_branch> → <main_branch>  (通过 <repo_type> API / gh CLI)
├── ⏸️  等待你确认 PR1 合并完成  ← 阻塞点 1
├── ✅ git checkout <main_branch> && git pull --ff-only
├── [打 tag + 平台 Release 在 <main_branch> 上发生]
├── ✅ git checkout <release_branch>
├── ✅ 创建 PR2: <release_branch> → <develop_branch>  (回流 release 准备 commit)
└── ⏸️  等待你确认 PR2 合并完成  ← 阻塞点 2(未合并不影响 tag,仅影响 develop 是否同步)

🏷️  Git Tag
<若 push_tag_first == true>   # push_tag_first 决策矩阵见 §9
├── ✅ git tag -a <version>  (annotated,本地创建)
└── ✅ git push origin <version>  →  推到 <repo_type>
<若 push_tag_first == false>
└── ❌ 不创建本地 tag(平台 Release API 会根据 target_commitish 在 <main_branch> HEAD 生成 lightweight tag)
...
> 对于 `flow_mode == "release-branch"`,跳过本步骤,进入步骤 8.7。

### 8.7 发布分支前置流程（仅 `flow_mode == "release-branch"`）

在 `release/*` 或 `chore/release-*` 分支上执行 release 时,走"双 PR"流程。本步只处理 **PR1 之前 + PR1 等待合并** 的部分;PR2(回流到 develop)延后到 step 10.6 处理——原因是 tag 和 Release 必须在 main 代码已就位之后才能创建,把 PR2 放在 Release 之后可以确保 PR2 是真正"回流 release 成品"。

**整体时间线**:

```
[本步 8.7]  提交产物到 release 分支 → PR1 → 等合并 → checkout main
[step 9]    在 main 上打 tag(或委托给 Release API 创建)
[step 10]   创建平台 Release
[step 10.6] checkout release 分支 → PR2 → 等合并(回流到 develop)
```

**为什么 PR1 在前,PR2 在后**:先合 PR1 保证 tag 落在干净的 main HEAD 上(只含本次发布内容);PR2 把 changelog/SQL/回滚脚本回流到 develop,让下次 release 不重复产出这些文件。顺序颠倒会导致 tag 被 develop 上未准备发布的 feature 污染。
...
1. **提交产物到 release 分支**
   ...
   git add docs/migrations/released/<version>.sql \
           docs/migrations/released/<version>.rollback.sql \
           docs/changelogs/<version>.md
   # 步骤 6 的 git rm 已将源 SQL 删除放入暂存区,无需再 add
   git commit -m "chore(release): prepare <version>"
   git push origin <release_branch>
   ```
...
2. **创建 PR1: release 分支 → main**
   ...
   **⚠️ 复用 PR 前必须检查状态**(规则同 §8.5 步骤 2):查询 `state=open` 的 PR 才能复用,`merged`/`closed` 必须新建。

   根据 `repoType` 调用相应接口(`gitea` → API、`github` → `gh pr create`、`other` → 输出手动命令后终止)。

   - PR 标题:`chore(release): <version> → main`
   - PR Body:本次发布的需求清单 + changelog 正文摘要 + "合并后将在 `<main_branch>` HEAD 上创建 tag `<version>`,然后发 PR2 回流 `<develop_branch>`"
...
3. **等待用户确认 PR1 合并完成**
   ...
   ⏸️  PR1 已创建:<PR URL>
       ...
       合并完成后回复"继续"或"y"以继续后续流程(打 tag + 创建 Release + PR2 回流)。
       回复其他内容将中止本次发布(已创建的 SQL/changelog/PR1 会保留)。
   ```
...
4. **拉取主分支最新状态**
   ...
   若文件不存在说明 PR1 未真正合并或合并的是旧版本:
   - **不得**尝试 `git checkout <release_branch> -- docs/changelogs/<version>.md` 这种补丁式操作
   - **不得**直接 push 主分支
   - 警告用户 PR1 状态异常,回到步骤 2 重新创建新 PR1,再次等待合并确认

5. **继续执行 step 9(打 tag)和 step 10(创建 Release)**,此时:
   - `tag_target = main_branch`
   - tag 的创建方式由 §9 的 `PUSH_TAG_FIRST` 决策矩阵决定(Gitea 非 draft 场景由 API 从 target_commitish 现场生成 lightweight tag,其他场景按矩阵处理)
   - step 10 完成后进入 **step 10.6** 处理 PR2

> 对于 `flow_mode == "direct"`,跳过本步骤,进入 step 8.8。
> 对于 `flow_mode == "cross-branch"`,跳过本步骤(已在 step 8.5 处理)。
...
> 跨分支模式已在步骤 8.5 中处理,发布分支模式已在步骤 8.7 中处理,均跳过本步骤。
...
```bash
# tag 应该打在 tag_target 分支上
#   direct 模式        = 当前分支(main)
#   cross-branch 模式  = main_branch
#   release-branch 模式 = main_branch
...
# push_tag_first 决策矩阵（本地是否需要先创建 annotated tag 并 push）:
#   非 draft + gitea   → false (Gitea Release API 在非 draft 时会从 target_commitish 现场生成 lightweight tag,本地推是冗余的)
#   非 draft + github  → true  (避免 GitHub API 在 tag 缺失时用默认分支当 target,打错分支)
#   非 draft + other   → true  (无 Release API,本地 annotated tag + push 是唯一路径)
#   draft + gitea      → true  (Gitea Release API 在 draft=true 时要求 tag 先存在,否则 422 "Release is has no Tag")
#   draft + github     → false (gh release create --draft --target <tag_target> 由平台懒创建,publish 时生成 lightweight tag)
...
覆盖三种情况:非 draft + github、非 draft + other、draft + gitea。
...
覆盖两种情况:draft + github、非 draft + gitea。

```bash
if [ "$IS_DRAFT" = "true" ] && [ "$REPO_TYPE" = "github" ]; then
    echo "📌 draft + github 模式:跳过本地 tag 创建,gh release create --draft 将懒创建"
    echo "   target_commitish = ${tag_target}(publish 时在该分支 HEAD 上生成 lightweight tag)"
elif [ "$IS_DRAFT" = "false" ] && [ "$REPO_TYPE" = "gitea" ]; then
    echo "📌 非 draft + gitea 模式:跳过本地 tag 创建,Gitea Release API 将在 target_commitish 现场生成"
    echo "   target_commitish = ${tag_target}(API 会在该分支 HEAD 上生成 lightweight tag)"
fi
```

tag 的创建完全托付给 step 10 的 Release API:
- **draft + github**:用户在 GitHub 上点 Publish 时,平台根据 draft 记录的 `target_commitish` 生成 lightweight tag
- **非 draft + gitea**:step 10 调用 Gitea Release API 时传 `target_commitish=tag_target`,Gitea 在创建 Release 的同时于该分支 HEAD 生成 lightweight tag

---

**`tag_target` 定义**：
- `direct` 模式：`current_branch`（main）
- `cross-branch` 模式：`main_branch`
- `release-branch` 模式:`main_branch`

**为什么非 draft + github 要先 push tag**:GitHub Release API 在 tag 不存在时会用 `target_commitish` 现场创建 tag。如果 `target_commitish` 缺省为默认分支(可能是 develop),tag 就被打错。先推已存在的 annotated tag,API 直接引用,不再创建。

**为什么非 draft + gitea 不 push tag**:Gitea Release API 在 `draft=false` 时会从 `target_commitish` 现场生成 lightweight tag,而我们在 step 10 显式把 `target_commitish` 设为 `tag_target`(主分支名),不会打错分支。本地再 push annotated tag 是**冗余**——会出现"本地 annotated、远程 lightweight"两种类型 tag 同名冲突(或者被 push 覆盖)。省掉这一步既简化流程、又与用户直觉一致(tag 在创建 Release 时一并出现)。

**为什么 Gitea 的 draft 模式反而要先 push tag**:Gitea Release API 在 `draft=true` 时不会为你创建 tag——必须先存在,否则返回 `422 Release is has no Tag`。这是 Gitea 与 GitHub 的关键差异(GitHub 的 draft release 可以引用未来的 tag,Gitea 不可以),因此 Gitea 上的 draft 闸门价值比 GitHub 弱一些:放弃 draft 需同时清理 tag(`git push --delete origin <tag> && git tag -d <tag>`)。
...
**前置条件**:
- **draft + gitea**:step 9 已 push annotated tag(API 要求 tag 先存在)。此时 `target_commitish` 仅作为备用信息
- **非 draft + gitea**:step 9 **未** push tag——API 会根据下方传入的 `target_commitish=tag_target` 在该分支 HEAD 生成 lightweight tag

**调用 API**：

⚠️ **重要：body 必须用 `jq` 从文件构造 JSON**，不要手工拼接 JSON 字符串，否则 emoji（🐛、🔗、🗄️ 等 4 字节 UTF-8）会在 shell 双引号转义过程中退化成 `�` replacement char。

```bash
# target_commitish 统一传分支名 tag_target:
#   draft + gitea 场景   → tag 已存在,target_commitish 仅作为备用信息
#   非 draft + gitea 场景 → tag 未创建,Gitea 会在该分支 HEAD 生成 lightweight tag
TARGET_COMMITISH="${tag_target}"

# 用 jq 从 changelog 文件读取 body 并安全构造 JSON（保留 UTF-8/emoji）
jq -n \
  --arg tag "<version>" \
...
> 关键点：
> - `--rawfile body <path>`：jq 直接读文件为字符串，不经过 shell 参数展开，UTF-8 原样保留
> - `--data-binary @file`：curl 按二进制流上传文件，不做换行/编码转换
...
无论 `flow_mode` 是 `direct`、`cross-branch` 还是 `release-branch`,在 tag/Release 完成后都要切回命令启动时所在的分支(记录为 `start_branch`):
...
**用途**:
- `cross-branch` 模式:从 `main` 切回 `developBranch`,避免用户残留在主分支上误操作
- `release-branch` 模式:从 `main` 切回 `release_branch`,准备执行 step 10.6 的 PR2 回流
- `direct` 模式:若当前已在 `start_branch`,等同空操作
- 切换前若工作区有脏改动(不应该发生,但防御性检查),警告并跳过切换

### 10.6 PR2 回流到 develop(仅 `flow_mode == "release-branch"`)

tag 和 Release 已在 `main_branch` 上完成。现在需要发 PR2 把 release 准备 commit(changelog + SQL + 回滚)回流到 `develop_branch`,避免下次 release 重复生成这些文件。

**前置条件**:step 10.5 已把当前切回到 `release_branch`。
...
2. **创建 PR2: release 分支 → develop**
   ...
   复用/新建规则同 §8.5 步骤 2(`state=open` 才复用,`merged`/`closed` 必须新建)。
...
3. **等待用户确认 PR2 合并完成**(非阻塞式)
   ...
   ⏸️  PR2 已创建:<PR URL>
       ...
       回复"跳过"或其他内容,PR2 保留等你后续手动合并,命令直接进入最终报告。
...
4. **进入 step 11 最终报告**,把 `pr2_status` 一并打印。

> 对于 `flow_mode == "direct"` 或 `"cross-branch"`,跳过本步骤。
