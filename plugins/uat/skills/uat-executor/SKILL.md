...
**浮层查找原则**：不在当前容器内查找，在 `body` 全局查找新出现的浮层元素（按组件库特征或新增 DOM 节点检测）。
...
执行测试时，主动观察并记录 flow 文档**未覆盖**但实际存在的内容，作为"发现项"积累。

### 哪些情况触发记录

| 发现类型 | 示例 | 记录格式 |
|---------|------|---------|
| 新增字段 | 页面有「标签」字段但 flow 未提及 | `[新字段] 标签 — 在 S02 新增表单中发现` |
| 新功能入口 | 列表页有「批量导入」按钮未在 flow 中 | `[新功能] 批量导入按钮 — 未覆盖` |
| 流程差异 | 步骤 3 预期弹出确认框，实际直接删除 | `[流程差异] S04 删除无二次确认 — 流程文档需更新` |
| API 行为 | 接口返回额外字段 / 错误码在 flow 未描述 | `[接口行为] 400 时返回 field_errors 对象 — 未覆盖` |
| 提示文本 | 错误提示文字与预期不同 | `[文本差异] S02 步骤 3 实际提示「用户名或密码错误」而非「密码错误」` |

**不要因为发现项中断测试**：记录后继续执行，汇总到最后处理。

### 场景结束后的即时处理

每个场景执行完毕后（记录 PASS/FAIL/SKIP 后立即执行）：
...
```
💡 S02 执行中发现 2 项：
   [流程差异] 步骤 4 的错误提示为「用户名或密码错误」（flow 文档写的是「密码错误」）
   [新字段]   表单中有「备注」字段，flow 未覆盖

立即更新流程文档？(y/n/跳过全部)
```

- **y**：就地更新 flow 文档中本场景对应的节，加注 `⚠️ 已修正（YYYY-MM-DD）`，然后继续下一场景
- **n**：发现项暂存，继续下一场景，全部跑完后汇总再处理
- **跳过全部**：本次执行剩余发现项全部仅写报告，不修改 flow 文档
...
| 失败原因 | 判断依据 | 处理方式 |
|---------|---------|---------|
| 代码 Bug | 功能确实不工作（按钮无响应、接口报错、数据未保存） | 截图，记入报告，提示 `/uat:bug` 上报 |
| 用例描述有误 | 功能正常工作，只是预期描述与实际不符（文本、顺序、路径） | 提示更新流程文档，不作为 bug 上报 |
...
❌ S03 失败：点击保存后无提示
  ...
  原因分析：功能正常执行（数据已保存），但提示为 Toast 而非弹窗。
  → 疑似用例描述有误（实际行为与预期描述不符，非代码 bug）

  [1] 更新用例描述（将「弹窗提示」改为「Toast 提示」）
  [2] 记为代码 Bug，后续 /uat:bug 上报
  [3] 保持原样，继续
```

### 全部场景结束后的汇总处理

所有场景结束后，若仍有未处理的发现项（选 n 暂存的），汇总展示：

```
─────────────────────────────────
💡 剩余发现项（3 项）：
   [新字段]   S02 「备注」字段未覆盖
   [文本差异] S05 按钮文字「确认」而非「确定」
   [新功能]   列表页发现「批量删除」按钮
─────────────────────────────────
批量处理：(a)全部更新到 flow 文档  (n)全部写报告不修改
```

用户选 **a**：
- `[新字段]`：在对应场景步骤末尾追加该字段的验证步骤（标注「执行发现，待补充测试数据」）
- `[文本差异]`：更新预期结果中的文本
- `[新功能]`：在 flow 文档末尾追加新场景草稿（标注「待完善，由 /uat:run 发现」）

用户选 **n**：发现项仅写入报告文件的「发现记录」节，不修改 flow 文档。
...
| 类型 | 场景 | 内容 | 是否已补充到 flow |
|------|------|------|-----------------|
| 新字段 | S02 | 「标签」字段未在 flow 中覆盖 | ✅ 已补充 |
| 流程差异 | S04 | 删除无二次确认 | ✅ 已修正 |
| 文本差异 | S02 | 错误提示文本不符 | ❌ 未补充 |
```
