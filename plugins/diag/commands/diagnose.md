...
- `--service` 已指定 → 校验存在后使用
- 未指定 → 按描述自然语言匹配（如「订单接口」→ `order-api`）。只有一个服务、唯一匹配或描述明确要查全部 → 直接使用，在报告头注明「目标服务：xxx（按描述匹配，不对请加 `--service` 重跑）」；匹配不到或有多个候选 → 列出服务清单并**结束本轮**，请用户用 `/diag:diagnose <报错描述> --service=<name>` 重新发起

> 不在本轮停下等用户选服务：本命令 `model: best` 只作用于当轮，用户回复后的堆栈分析与根因判断会落到会话模型上；带 `--service` 重跑是新的一轮。
