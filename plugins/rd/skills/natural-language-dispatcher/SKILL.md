---
name: natural-language-dispatcher
description: 自然语言需求调度器。当用户用自然语言描述下列意图时自动触发并映射到对应命令： - 需求文档：新增/新建/创建需求、修改/编辑/变更需求（带编号） - 修复与开发：修 bug、修复报错、优化/重构/升级/统一代码、快速修复、小功能 - 状态流转：开发/测试/提审/评审通过/评审驳回/完成/归档需求（必须带编号） - 版本 PR：规范提交、创建 PR、审查 PR、按评论改代码、合并 PR、拉 PR 评论 - Issue 操作：提/建/开 issue、评论讨论、关闭/重开/查看/列出 issue - URL 识别：识别 issue/PR 链接并映射到对应命令 - 确认偏好：开启/关闭/查询 git commit 前的原生确认弹框
---

# 自然语言需求调度器

解析用户自然语言意图，映射到对应命令。本技能只做**意图识别与命令映射**，解析完成后读取对应技能文件按原流程执行。

> 唯一例外：§七「确认偏好开关」不对应任何命令，由本技能直接执行文件动作。

---

## 编号解析规则

| 输入 | 解析 |
|------|------|
| `REQ-025` / `REQ025` | REQ-025 |
| `QUICK-003` / `QUICK003` | QUICK-003 |
| 纯数字 `025` / `3` | REQ-XXX（补零到 3 位） |
| `#42` / `issue 42` | issue `#42`，映射为 `--from-issue=#42` |

纯数字默认 REQ。当前分支暗示 QUICK（如 `fix/QUICK-XXX-...`）时优先 QUICK。无法确定时询问。

---

## 一、需求文档操作

### 新增需求 → `/rd:new`

触发："新增需求"、"新建需求"、"创建需求"（不要求编号，系统自动生成）。

提取字段：标题、类型（`--type`）、模块（`--module`）、issue（`--from-issue`）。

### 修改需求 → `/rd:edit`

触发："修改/编辑/变更需求" + 编号（缺编号不触发）。

---

## 二、修复与开发（无文档）

### 修 bug → `/rd:fix`

触发："修/修复/修一下" + "bug/问题/报错/异常/错误" + 具体描述。

`--auto` 触发词："一键修"、"自动修"、"修完直接发 PR"、"不用确认"等。识别到 `--auto` 时说明能力边界（跳过哪些确认、哪些无法跳过）。

不触发：提问/讨论语气（"这个 bug 怎么修"）。

### 优化/重构/升级 → `/rd:do`

触发：祈使语气动词（优化、重构、升级、统一、整理、抽取、迁移）+ 具体对象。

不触发：讨论语气（"可以优化"）、无对象（"优化一下"）。

### 快速修复/小功能 → `/rd:new-quick`

触发："快速修复"、"小功能"、"快速需求"等。会创建 QUICK 文档（与 `/rd:fix` 无文档的区别）。

---

## 三、需求状态流转（必须带编号）

| 自然语言 | 映射 |
|---------|------|
| "开发/开始开发" + 编号 | `/rd:dev` |
| "测试/开始测试" + 编号 | `/rd:test` |
| "提审/提交评审" + 编号 | `/rd:req-review` |
| "评审通过" + 编号 | `/rd:req-review pass` |
| "评审驳回/驳回" + 编号 | `/rd:req-review reject` |
| "完成/归档/做完了" + 编号 | `/rd:done` |

---

## 四、版本与 PR 操作

| 自然语言 | 映射 |
|---------|------|
| "规范提交"、"提交代码" | `/rd:commit` |
| "创建 PR"、"提 PR" | `/rd:pr` |
| "审 PR"、"审查 PR"、"审代码" | `/rd:review` |
| "拉 PR 评论"、"看看 PR 评论" | `/rd:pr comments`（只读展示） |
| "处理 PR 反馈"、"按评论改"、"照评论改代码" | `/rd:review comments` |
| "合并 PR" | `/rd:pr merge` |
| "看看 PR"、"PR 状态" | `/rd:pr status` |

`/rd:review` 的 `--auto` 触发词："自动审查"、"审查并提交"、"不用确认"等。

> "评审" + 需求编号是需求评审（`/rd:req-review`），"审查/审" + PR 是代码审查（`/rd:review`）；两者都没带、只说"评审一下"时询问。
> `/rd:pr merge` 和 `/rd:review comments` 各自交互点不走 `--auto`。

---

## 五、Issue 操作

| 自然语言 | 映射 |
|---------|------|
| "提/建/开 issue: <标题>" | `/rd:issue new <标题>` |
| "给 #N 回复/评论 <内容>" | `/rd:issue comment N <内容>` |
| "关闭/close issue N" | `/rd:issue close N` |
| "重开/reopen issue N" | `/rd:issue reopen N` |
| "看下 issue N" | `/rd:issue show N` |
| "列出 issue" | `/rd:issue list` |

编辑类："给 issue N 加标签 X" → `/rd:issue edit N --add-labels=X`

---

## 六、Git 平台 URL 识别

### 识别模式

支持完整 URL 和纯路径：`owner/repo/issues/N`、`owner/repo/pulls/N`。`pull` 和 `pulls` 均识别为 PR。

### 仓库匹配

解析 `owner/repo` 与 `git remote get-url origin` 对比。不匹配则提示切换仓库，不跨仓库执行。

### Issue URL

- **仅 URL**（无动词）→ 展示操作选项（修 bug / 创建需求 / 快速修复 / 智能开发 / 查看 / 评论 / 关闭）
- **URL + 动词**（如"修 .../issues/169"）→ 直接映射到对应命令

### PR URL

先检查当前分支的 PR 编号是否匹配 URL 中的编号，不匹配则提示切换分支。

- **仅 URL** → 展示选项（查看状态 `/rd:pr status` / 审查 `/rd:review` / 看评论 `/rd:pr comments` / 按评论改 `/rd:review comments` / 合并 `/rd:pr merge`）
- **URL + 动词** → 直接映射

### 优先级

动词 + URL > 仅 URL > 动词 + 需求编号

---

## 七、确认偏好开关

用户自然语言开/关 Bash 原生确认弹框，不走命令映射。

**执行动作**：
- 开启 → `mkdir -p .claude && touch .claude/.req-confirm-commit`，写入 memory
- 关闭 → `rm -f .claude/.req-confirm-commit`，更新 memory
- 查询 → 检查 marker 是否存在，告知状态

> 只影响 Bash 侧 Hook，不拦截 Write/Edit。marker 已加入 `.gitignore`。

---

## 不触发的情况

- 查询/展示类："看一下025需求" → 走 `/rd:show`
- 讨论/提问："可以优化"、"怎么修"、"是不是该重构了"
- 缺必要信息："修改需求"无编号、"优化一下"无对象
- 涉及插件自身维护：修改模板、优化技能
- URL 指向其他仓库、非 issue/PR 路径、纯域名
- 用户消息以 `/rd:` 等斜杠命令开头

---

## 执行流程

1. **确认意图**：一行说明，如 `🧠 识别：/rd:fix 登录超时`
2. **读取对应技能文件**，按原流程执行
3. **传入解析出的参数**
4. **后续与原命令完全一致**

特殊情况：多意图逐个询问、有歧义先确认、命令需用户输入时进入原交互流程。

---

## 用户输入

$ARGUMENTS
