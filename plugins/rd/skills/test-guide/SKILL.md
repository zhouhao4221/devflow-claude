---
name: test-guide
description: 测试引导助手。在执行 /rd:test、/rd:test_regression 或 /rd:test_new 命令时触发。 支持运行已有自动化测试和创建新测试用例。
---
...
| 命令 | 用途 | 说明 |
|-----|------|------|
| `/rd:test_regression` | 运行已有测试 | 执行现有自动化测试用例，回归验证 |
| `/rd:test_new` | 创建新测试 | 为新功能编写 UT/API/E2E 测试用例 |
| `/rd:test` | 综合测试 | 先运行回归，再补充新测试 |
...
补充注入（存在则读，缺失静默跳过）：
- 运行 / 回归测试（`/rd:test`、`/rd:test_regression`）→ Read `docs/prompt/testing.md`（运行命令、文件位置、环境依赖）
- 创建新用例（`/rd:test_new`）→ Read `docs/prompt/test-generation.md`（用例编写规范、覆盖要求）
...
```bash
/rd:test_regression              # 全量回归
/rd:test_regression --changed    # 仅测试变更相关
/rd:test_regression --module=xxx # 指定模块
/rd:test_regression --failed     # 仅运行上次失败的
```
...
💡 下一步：
- 修复问题后重新测试：/rd:test_regression --failed
```
