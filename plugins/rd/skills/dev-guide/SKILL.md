---
name: dev-guide
description: 开发引导助手。仅在执行 /rd:dev 命令时触发。按项目分层架构引导开发。
---

# 开发引导助手

仅在 `/rd:dev` 命令执行时激活，负责实现方案生成、开发引导和进度跟踪。
...
- QUICK-XXX：跳过评审；方案已在 `/rd:new-quick` 确认，模板无「十一、实现方案」章节，不再生成方案
...
- **架构文件**：Read `docs/prompt/architecture.md`（不存在时打印非阻塞警告，建议 `/rd:init --reinit`）
...
草稿/开发中/测试中 → 允许。已完成 → readonly 允许，primary 警告。状态机唯一定义见 `_storage.md`「双轨状态机」。
...
所有步骤完成后输出摘要（步骤数、新增/修改文件数），提示下一步 `/rd:test`。
