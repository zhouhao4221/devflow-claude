...
if [ -n "$LEGACY_CONFIG" ]; then
    ...
    echo ""
    echo "执行 \`/rd:migrate\` 把 DevFlow 字段搬到 \`.devflow/\`（密钥进 \`settings.local.json\`）。"
    exit 0
fi
...
if [ -z "$PROJECT_NAME" ]; then
    ...
    echo ""
    echo "1. \`/rd:init <project-name>\` — 初始化需求项目（创建 \`docs/requirements/\`、生成 PRD、绑定仓库）"
    echo "2. \`/rd:branch init\` — 配置分支策略（GitHub Flow / Git Flow / Trunk-Based）"
    echo ""
    echo "💡 完成后即可 \`/rd:new <标题>\` 创建第一个需求。输入 \`/rd:help\` 查看完整教程。"
    exit 0
fi
...
if [ -n "$SOURCE_ERROR" ]; then
    echo "- ⚠️ 只读仓库未绑定主仓（\`requirementSource.path\` 缺失或路径不存在），执行 \`/rd:use <primary-repo-path>\` 重新绑定"
elif [ -n "$REQ_ID" ]; then
    if [ -n "$REQ_FILE" ]; then
...
if [ -z "$HAS_BRANCH_STRATEGY" ]; then
    echo ""
    echo "⚠️ 尚未配置分支策略，建议执行 \`/rd:branch init\` 选择分支模型（GitHub Flow / Git Flow / Trunk-Based）"
fi

echo ""
echo "💡 可用命令：\`/rd:req\` 列表 · \`/rd:status\` 详情 · \`/rd:dev\` 继续开发"
