...
>
> 同伴文档（同目录，按需 Read；此处不用链接，避免把整组共享文件拖进上下文）：`_storage.md`（存储与配置）、`_branch.md`、`_issue.md`、`_granularity.md`、`_claude-md.md`。
...
| `/rd:dev` | 评审通过 → 开发中（REQ）/ 草稿 → 开发中（QUICK） | 首次进入时自动更新 |
| `/rd:test` | 开发中 → 测试中 | 测试完成后自动更新 |
| `/rd:done` | 测试中 → 已完成 | **必须明确确认（y/n）** |
...
> `--auto` 模式标记（`.claude/.req-auto`）仍由 `/rd:fix --auto` 等流程负责建立/清理；在 marker 启用拦截时它负责让 Hook 放行自动化流水线。
...
4. **`/rd:done` 等显式 y/n 场景** — 由命令层提示，不依赖 Hook
...
完整定义（写入命令、验证章节、放行标注）见 `_storage.md`「双轨状态机」。
