...
- 运行 `/rd:migrate`（搬运 DevFlow 字段到 `.devflow/`，密钥进 `settings.local.json`）
- 重新运行 `/rd:init --reinit` 或 `/rd:branch init`
...
| 待评审 | `/rd:req-review`（含 AI 预审） | ✅ | — |
| 评审通过 / 评审驳回 | `/rd:req-review pass` / `reject` | ✅ | — |
| 开发中 | `/rd:dev`（首次进入） | ✅ | ✅ |
| 测试中 | `/rd:test`（完成时） | ✅ | ✅ |
