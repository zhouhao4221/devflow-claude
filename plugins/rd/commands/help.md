...
章节：
 1. 安装与初始化（含架构描述、分支策略、Gitea Token 配置、reinit、升级迁移 v2→v3 / req→rd）
 2. 创建需求
 3. 评审流程
 4. 开发阶段（含分支管理、PR 创建）
 5. 测试阶段
...
Sections:
 1. Installation & initialization (architecture, branch strategy, Gitea token, reinit, upgrade migration v2→v3 / req→rd)
...
섹션:
 1. 설치 & 초기화 (아키텍처, 브랜치 전략, Gitea Token, 재초기화, 업그레이드 마이그레이션 v2→v3 / req→rd)
 2. 요구사항 생성
 3. 리뷰 플로우
...
示例：
- `/rd:help` → 中文章节索引（或 `.devflow/settings.local.json` 中设定的语言）
