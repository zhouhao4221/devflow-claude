...
| 选项 | 说明 | 示例 |
|-----|------|------|
| `--all` | 全量回归（默认） | `/rd:test_regression --all` |
| `--changed` | 仅测试变更相关 | `/rd:test_regression --changed` |
| `--module=<name>` | 指定模块 | `/rd:test_regression --module=user` |
| `--failed` | 仅运行上次失败的 | `/rd:test_regression --failed` |
| `--type=<ut\|api\|e2e>` | 指定测试类型 | `/rd:test_regression --type=ut` |
| `--verbose` | 显示详细输出 | `/rd:test_regression --verbose` |
| `--coverage` | 生成覆盖率报告 | `/rd:test_regression --coverage` |
| `--skip-env` | 跳过环境检查（仅 UT） | `/rd:test_regression --type=ut --skip-env` |
...
下一步操作：
- 修复后重新测试：/rd:test_regression --failed
- 查看失败详情：/rd:test_regression --verbose
...
```bash
/rd:test_regression --ci --output=junit.xml
```
