...
```
/rd:update-template [模板名称] [--force]
```

**示例：**
- `/rd:update-template` - 交互选择要更新的模板
- `/rd:update-template requirement` - 更新需求模板
- `/rd:update-template all` - 更新所有模板
- `/rd:update-template requirement --force` - 强制覆盖，跳过对比确认
...
- 需求根目录 `requirementsDir`（缺省 `docs/requirements`）不存在 → 提示「本地需求目录不存在，请先执行 /rd:init <project-name>」，终止
...
| 错误场景 | 处理方式 |
|---------|---------|
| `${CLAUDE_PLUGIN_ROOT}` 为空或目录不存在 | 提示：无法定位插件目录，请用 `/plugin` 确认 rd 插件已安装并执行 `/reload-plugins` |
| 插件 templates/ 目录不存在 | 提示：插件模板目录不存在，插件安装可能不完整 |
| 本地需求目录不存在 | 提示先执行 `/rd:init` |
| 只读仓库 | 提示只读仓库不支持更新模板 |
| 用户取消 | 提示已取消，不做任何修改 |
