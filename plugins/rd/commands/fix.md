...
> **1.4–1.5 交给 `planner`（Fable）**：prompt 内联任务类型 `fix 根因+修复`、1.1 问题分析、1.2 定位清单、1.3 命中的关联需求节选、error-diagnosis.md 的规范（有则）、1.4 与 1.5 的输出骨架；主会话核对根因的 file:line 依据后展示。**planner 失败**（Agent 返回 API 错误，如 Fable 额度用尽、未开通、云厂商未上架；或超轮仍无结论）→ 不重试，主会话用当前模型按同一骨架自己分析，首行注明 `⚠️ Fable 不可用，本分析由当前模型生成`。
...
修改完成后按 `_verify.md`（步骤 1.5 已链接）执行验证：编译/lint → 相关测试回归（派 `test-runner`）→ 无测试时询问是否补 UT（修复类型）。测试失败最多修 2 轮，不得改断言迎合；仍失败停下交给用户，不进入步骤 4。UI 可观察的修复点先按 `_verify.md`「自主验收」派 `ui-verifier` 确认，剩余才留给用户手动验证。`--auto` 模式下「是否补 UT」按否处理，记「无测试覆盖」；自主验收照常执行（无需交互），出现 FAIL 时按失败处理停下，不进入 4.5 自动串联。
