...
**`--auto` 模式**：跳过"等待用户确认"，直接展示方案后进入步骤 2（全程在命令轮内，修复代码也跑 Fable；小 bug 改动少，接受）。

**非 `--auto` 模式**：展示方案后**结束本轮回复**，等用户下一条消息；不用 AskUserQuestion 确认。本命令 `model: best` 只作用于当轮：同轮内确认会让后续写代码也跑 Fable，结束本轮后回到会话模型。用户可以：
- 确认方案 → 进入步骤 2
- 补充信息 / 调整方向 → AI 重新分析
- 放弃 → 结束，不创建分支
