...
**`--auto` 非交互模式**：跳过修复方案确认、issue 关闭询问，修复完成后自动串联 `/rd:commit` + `/rd:pr`。
...
1. 读需求目录（按 `_storage.md` 解析）`active/` 全部与 `completed/` 最近 10 个文档，每份只取首行标题与元信息「模块」行（~500 token）
2. 用 bug 相关文件路径 + 关键词在标题 / 模块中模糊匹配（标题含关键词 或 模块与 bug 相关 → 命中）
...
问题在界面上可观察、且满足 `_verify.md`「自主验收」前置三条时，**根因分析前先复现**：把问题写成验证项（入口 / 操作 / **期望的正确行为**）派 `ui-verifier`，规则见 `_verify.md`「复现与确认现状」。描述缺入口或操作时先追问再派。

- **FAIL（已复现）** → 期望 vs 实际与探针路径内联进 1.4 的 planner prompt，作为根因依据
- **PASS（无法复现）** → 不进 1.4，停下问用户环境、数据、操作步骤；补充后可重派一次；仍 PASS → 让用户选「不带复现继续修（分析输出标注「未复现」）」或「停止」；选「停止」时删除本次复现探针（断言的是不存在的问题，留着会拦下下次 `/rd:fix` 的工作区检查）。`--auto` 模式下无法复现即停止整个流程并报告
- **BLOCKED / 前置不满足 / 非 UI 问题** → 跳过复现，分析输出注明「未复现：<原因>」，按原流程继续

#### 1.4 根因分析

> **1.4–1.5 交给 `planner`（Fable）**：prompt 内联任务类型 `fix 根因+修复`、1.1 问题分析、1.2 定位清单、1.3 命中的关联需求节选、1.3.5 复现结果（期望 vs 实际，无则注明未复现原因）、error-diagnosis.md 的规范（有则）、1.4 与 1.5 的输出骨架；主会话核对根因的 file:line 依据后展示。**planner 失败**（Agent 返回 API 错误，如 Fable 额度用尽、未开通、云厂商未上架；或超轮仍无结论）→ 不重试，主会话用当前模型按同一骨架自己分析，首行注明 `⚠️ Fable 不可用，本分析由当前模型生成`。
...
- 确认方案 → 进入步骤 2
- 补充信息 / 调整方向 → AI 重新分析
- 放弃 → 结束，不创建分支；1.3.5 生成过复现探针时一并删除
...
有未提交改动时终止，提示先 commit 或 stash。**例外**：1.3.5 生成的复现探针（`acceptance/<分支 slug>/repro-*` 未跟踪文件）不算改动——它属于本次修复，建分支时随工作区带到新分支。
...
修改完成后按 `_verify.md`（步骤 1.5 已链接）执行验证：编译/lint → 相关测试回归（派 `test-runner`）→ 无测试时询问是否补 UT（修复类型）。测试失败最多修 2 轮，不得改断言迎合；仍失败停下交给用户，不进入步骤 4。1.3.5 有复现探针时先重跑同一探针（应 FAIL → PASS）；其余 UI 可观察的修复点按 `_verify.md`「自主验收」派 `ui-verifier` 确认，剩余才留给用户手动验证。`--auto` 模式下「是否补 UT」按否处理，记「无测试覆盖」；自主验收照常执行（无需交互），出现 FAIL 时按失败处理停下，不进入 4.5 自动串联。
...
- 复现探针：FAIL → PASS（<路径>） | 未复现（<原因>） | 非 UI 问题
