...
- 多个候选 → 当前分支名含其中某个编号时直接用它；否则列出候选并**结束本轮**，请用户用 `/rd:dev <编号>` 重新发起（出方案前不停下等回复，见步骤 4 的闸门说明）
...
3. **多个分支**：当前分支就在列表里 → 直接用；否则列出分支并**结束本轮**，请用户 `git checkout <分支>` 后重跑 `/rd:dev`：
   ```
   此需求有多个开发分支：
   1. feat/REQ-025-backend
   2. feat/REQ-025-frontend
   切到要开发的分支后重跑 /rd:dev REQ-025
   ```
...
**确认闸门**：展示方案（首次进入附待建分支名）后**结束本轮回复**，等用户下一条消息再进入步骤 5；不进 Plan Mode，也不用 AskUserQuestion 确认。本命令 `model: best` 只作用于当轮：同轮内确认会让后续写代码也跑 Fable，结束本轮后回到会话模型。
