...
```
/rd:dev [REQ-XXX] [--reset]
```

- 省略编号时自动选择可开发的需求：「评审通过」「开发中」的 REQ，「草稿」「开发中」的 QUICK
- `--reset` 强制从头开始
...
- 指定编号 → 使用该需求
- 未指定 → 查找可开发需求（REQ：评审通过 / 开发中；QUICK：草稿 / 开发中）
- 多个候选 → 当前分支名含其中某个编号时直接用它；否则交互式选择（列出编号列表，用户输入编号）
...
| 当前状态 | 处理方式 |
|---------|---------|
| 草稿 | 拒绝开发，提示：`需求尚未评审，请先执行 /rd:req-review 提交评审` |
| 待评审 | 拒绝开发，提示：`需求正在评审中，请等待评审通过后再开发` |
| 评审驳回 | 拒绝开发，提示：`需求评审未通过，请先修改后重新提审：/rd:edit → /rd:req-review` |
| 评审通过 | 允许开发 |
| 开发中 | 允许继续开发 |
| 测试中 | 允许开发（可能是修复测试问题） |
...
3. **多个分支**：当前分支就在列表里 → 直接用；否则展示列表，让用户选择切入哪一个：
   ```
   此需求有多个开发分支，请选择：
...
4. 当前分支不是 `<BRANCH_FROM>` 时先 `git checkout <BRANCH_FROM>`（工作区已检查干净），让步骤 3、4 基于它的代码出方案
5. 分支名**不单独确认**，随步骤 4 的方案一起展示，用户确认方案时可一并改名：
   ```
...
REQ 读需求定义章节：需求描述、功能清单、业务规则、使用场景、接口需求、测试要点。QUICK 读「问题描述」「实现方案」「验证方式」。

### 4. 生成实现方案

> **QUICK 不走本步的生成流程**：方案已在 `/rd:new-quick` 生成并确认，且 QUICK 模板没有「十一、实现方案」章节——不生成、不新增章节，只展示文档「实现方案」请用户确认后进入步骤 5；`--reset` 时重新生成并回写到同一「实现方案」章节。以下仅 REQ。
...
基于需求文档和 CLAUDE.md 架构信息生成实现方案，填充「十一、实现方案」章节：
...
> **方案交给 `planner`（Fable）**：`code-scout` 返回后派 `planner`，prompt 内联任务类型 `dev 方案`、需求第二~五章要点（功能清单、业务规则、接口需求）、architecture.md 的分层与开发规范摘要、`code-scout` 的 file:line 清单、上面 11.1–11.4 的骨架。主会话逐条对照功能清单与分层复核返回的方案，缺项或越层直接修正（或 SendMessage 让它补），「存疑」项带给用户。**planner 失败**（Agent 返回 API 错误，如 Fable 额度用尽、未开通、云厂商未上架；或超轮仍无结论）→ 不重试，主会话用当前模型按同一骨架自己出方案，首行注明 `⚠️ Fable 不可用，本方案由当前模型生成`。规则见 `_delegate.md`「思考委派」。

如果需求文档「十一、实现方案」已有完整内容（非占位文本），直接展示并请用户确认。

展示方案（首次进入附待建分支名）后等用户确认，再进入步骤 5。

### 5. 更新状态和实现方案

> 仅 `primary` 仓库执行，`readonly` 仓库跳过此步骤（不修改需求文档）。

0. 首次进入且需建分支 → 先按步骤 2.5 情况 B 第 6 条建分支、写 `branch` 字段（用户改了名就用新名）
1. 首次进入 → 状态改为「开发中」并勾选生命周期「开发中」格（守卫要求状态与最后已勾格一致）
2. 仅 REQ：将实现方案写回需求文档的「十一、实现方案」章节（11.1 数据模型、11.2 API 设计、11.3 文件改动清单、11.4 实现步骤）
