...
**规模判断依据**（按 [`_granularity.md`](../shared/_granularity.md)「REQ / QUICK / 无文档的选择」的两问判定，本命令不另设文件数阈值）：

| 规模 | 条件 | 建议流程 |
|------|------|---------|
| 轻量 | 无需留痕，且不触及任何「需要他人点头」信号 | 直接执行（本命令） |
| 中等 | 需要留痕，但不需他人点头 | 建议 `/rd:new-quick` |
| 正式 | 命中任一「需要他人点头」信号（新 API、表结构、跨模块、下游可感知、破坏性变更） | 建议 `/rd:new` |

**规模为中等或正式时**：
```
此任务规模较大，建议使用正式流程以便追踪：

  /rd:new-quick <标题>    有文档记录的轻量任务
  /rd:new <标题>          正式需求（含评审、测试）
...
**方案交给 `planner`（Fable）**：prompt 内联任务类型 `do 方案`、第 1 步的意图与类型、refactoring.md 的约束（有则）、architecture.md 分层摘要、定位清单、下面的方案骨架；主会话复核后展示。**planner 失败**（Agent 返回 API 错误，如 Fable 额度用尽、未开通、云厂商未上架；或超轮仍无结论）→ 不重试，主会话用当前模型按同一骨架自己出方案，首行注明 `⚠️ Fable 不可用，本方案由当前模型生成`。
...
修改完成后按 [`_verify.md`](../shared/_verify.md) 执行：编译/lint → 相关测试回归（派 `test-runner`）→ 覆盖判断（无测试时按步骤 1 的类型分流：小功能/修复询问补 UT，其余记「无测试覆盖」并派生手动验证清单；清单中的 UI 可观察项先按 `_verify.md`「自主验收」派 `ui-verifier`，剩余才留给用户）。测试失败最多修 2 轮，不得改断言迎合；仍失败停下交给用户，不进入步骤 4。
