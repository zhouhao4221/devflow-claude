...
CURRENT_VER=$(jq -r '.version // "unknown"' "$SOURCE_PATH/plugins/rd/.claude-plugin/plugin.json" 2>/dev/null)
```
...
NEW_VER=$(jq -r '.version // "unknown"' "$SOURCE_PATH/plugins/rd/.claude-plugin/plugin.json" 2>/dev/null)
```
