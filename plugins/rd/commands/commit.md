...
**auto 模式**：若项目内存在 `.claude/.req-auto` 且 mtime 在 10 分钟内（由上游 `/rd:fix --auto` 等命令创建），**跳过交互式选择**，AI 根据 `git diff --cached` 的变更内容自动推断类型（如上游是 `/rd:fix` 则固定为"修复"）。
...
**Issue 关联：** 按 [_issue.md 的 Issue 读取优先级](../shared/_issue.md#issue-编号的读取优先级) 获取 issue 编号：先查需求文档 `issue` 字段，再查分支名 `-iN` 后缀。检测到 issue 编号时，在 commit message 末尾追加 `closes #N`。
