...
**必需**（缺失时 `/rd:dev` 降级为通用实现，质量明显下降）：

| 关键词 | 用途 | 依赖命令 |
|--------|------|---------|
| 技术栈 / 框架 / language | 生成代码时选择正确语言和库 | `/rd:dev` |
| 分层 / 层级 / layer | 确定实现的层级顺序（controller→service→repo 等） | `/rd:dev` |
| 目录 / 结构 / directory | 文件放置位置 | `/rd:dev` |
| 命名 / 规范 / convention | 函数名、文件名、变量名风格 | `/rd:dev` |

**推荐**（缺失时命令仍可运行，但生成结果可能与项目现有代码风格不一致）：

| 关键词 | 用途 | 依赖命令 |
|--------|------|---------|
| 测试 / test | 测试文件位置和规范 | `/rd:dev`、`/rd:test` |
| 接口 / API / endpoint | 接口层规范和请求格式 | `/rd:dev` |
| 错误处理 / error | 统一错误返回结构 | `/rd:dev` |

---

## docs/prompt/release.md

**可选文件**（缺失时 `/rd:release` 使用插件默认行为，跳过项目特有检查和步骤）：
...
**可选文件**（缺失时 `/rd:test` 使用内置默认值，可能不匹配项目实际测试配置）：
...
以下文件由 `/rd:init` 从 `templates/prompt-snippets/` 生成，遵循固定 5 节结构（什么时候用 / 必备输入 / 触发方式 / 优质输出标准 / 常见失败模式）。**均为可选文件**：存在则对应命令注入，缺失时命令降级为通用行为（不报错，仅 `/rd:update` 校验时提示推荐补齐）。
...
| `pr-review.md` | PR 评审关注点 | `/rd:review-pr` |
