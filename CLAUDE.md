...
### 确认操作规范（CRITICAL - 必须遵守）

当命令文件中出现 `⚠️🔴 强制确认` 或 `⚠️🟡 标准确认` 标记时，**你的回复必须在确认提示处结束**。具体规则：

1. **输出确认提示后立即结束当前回复** — 确认提示必须是你这条消息的最后内容，不得在同一条消息中继续执行任何后续操作
2. **等待用户在下一条消息中明确回复** — 用户必须显式回复 y/n，不得自行假设用户意图
3. **用户回复前禁止执行写入操作** — 不得提前调用 Write、Edit、Bash(git commit) 等工具
4. **不得合并或跳过确认** — 每个确认点必须独立等待用户回复

**确认级别**：
- 🔴 强制确认：不可逆操作（归档、删除、git commit、升级）
- 🟡 标准确认：写入操作（编辑文档、写入方案、开始编码）
- 🟢 轻量确认：可选操作（创建测试、查看建议）
