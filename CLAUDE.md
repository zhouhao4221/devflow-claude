...
命令文件中的确认操作采用**回车继续**模式，减少用户输入负担：

1. **展示摘要后，输出确认提示并立即结束当前回复**，等待用户在下一条消息中回复
2. **默认继续**：用户回复回车（空消息）或任意非 n 内容 = 确认继续
3. **输入 n 取消**：仅当用户明确输入 n 时才取消操作
4. **不得跳过确认** — 即使上下文意图明确，仍需展示摘要并等待用户回复

**确认级别**：
- 🔴 强制确认：不可逆操作（归档、删除、git commit、升级）— 展示摘要后等待回车
- 🟡 标准确认：写入操作（编辑文档、写入方案、开始编码）— 展示预览后等待回车
- 🟢 轻量确认：可选操作（创建测试、查看建议）— 直接执行，无需等待
