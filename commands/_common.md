...
确认操作采用**回车继续**模式：展示摘要后等待用户回复，回车或任意非 n 内容 = 继续，输入 n = 取消。

### 确认级别

| 级别 | 适用场景 | 要求 |
|------|---------|------|
| 🔴 强制确认 | 不可逆操作（归档、删除、覆盖、git commit） | 展示摘要后**立即结束回复**，等待用户回车继续或输入 n 取消 |
| 🟡 标准确认 | 写入操作（编辑文档、写入方案、开始编码） | 展示变更预览后**立即结束回复**，等待用户回车继续或输入 n 取消 |
| 🟢 轻量确认 | 可选操作（创建测试、查看建议） | 直接执行，无需等待 |

### 确认提示格式

当命令文件中标注了 `⚠️🔴 强制确认` 或 `⚠️🟡 标准确认` 时，按以下格式输出并**立即结束当前回复**：
...
1. **确认提示必须是当前回复的最后内容** — 输出后立即结束，不得继续执行后续步骤
2. **回车/空消息/任意非 n 内容 = 确认继续** — 默认继续，减少用户输入负担
3. **仅 n 取消** — 只有用户明确输入 n 时才取消操作
